...
static HSV SOLID_REACTIVE_CROSS_math(HSV hsv, int16_t dx, int16_t dy, uint8_t dist, uint16_t tick, HSV background_hsv) {
...
#        ifdef ENABLE_RAINBOW_BACKGROUND
    background_hsv.h = tick % 256; // Rainbow effect
    background_hsv.s = 255;
    background_hsv.v = 128;
#        else
    background_hsv.h = 128; // Static background color (e.g., blue)
    background_hsv.s = 255;
    background_hsv.v = 128;
#        endif
...
bool SOLID_REACTIVE_CROSS(effect_params_t* params) {
    HSV background_hsv = get_background_color(g_rgb_timer);
    return effect_runner_reactive_splash(qsub8(g_last_hit_tracker.count, 1), params, 
                                         [background_hsv](HSV hsv, int16_t dx, int16_t dy, uint8_t dist, uint16_t tick) {
                                             return SOLID_REACTIVE_CROSS_math(hsv, dx, dy, dist, tick, background_hsv);
                                         });
}
#            endif
